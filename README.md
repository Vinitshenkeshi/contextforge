...
The dev server seeds itself with a bundled fictional support KB ("Nimbus Notes" - billing, API keys, rate limits, 2FA, webhooks, exports, and more) the first time it's queried, so the chat demo on `/` works immediately with no setup. Try `/ingest` to add your own markdown/PDF/URL, and `/admin` to see the review queue fill up when you ask something out of scope.
...
Vinit Shenkeshi is an AI/ML Engineer with over 4 years of experience building end-to-end machine learning and AI solutions. Specializing in NLP, RAG, and LLM orchestration, he focuses on creating scalable, production-ready systems that bridge the gap between raw data and actionable insights.
